...
for field in fields:
    ...
    for i in range(field_arrays.GetNumberOfArrays()-1): #Should not be -1, fix later **
...
warp.SetInputArrayToProcess(0, 0, 0, vtkDataObject.FIELD_ASSOCIATION_POINTS, 'Displacement')
warp.SetScaleFactor(1)
warp.Update()

warp_mapper = warp.GetOutput()
warp_mapper = vtkDataSetMapper()
warp_mapper.SetInputConnection(warp.GetOutputPort())

# Create an actor for the deformed mesh
warp_actor = vtkActor()
...
warp_actor.GetProperty().SetRepresentationToSurface()
warp_actor.GetProperty().SetPointSize(1)
warp_actor.GetProperty().EdgeVisibilityOff()
...
def visibility_change(event):
    ...
    if _id == "1":  # Mesh
        ...
        warp_actor.SetVisibility(_visibility)
    ctrl.view_update()
...
def update_mesh_color_by_name(mesh_color_array_idx, **kwargs):
    ...
    color_by_array(warp_actor, array)
    ctrl.view_update()

@state.change("warp_color_array_idx")
def update_warp_color_by_name(warp_color_array_idx, **kwargs):
    array = dataset_arrays[warp_color_array_idx]
    print(array)
    color_by_array(warp_actor, array)
    ctrl.view_update()
...
def update_mesh_color_preset(mesh_color_preset, **kwargs):
    ...
    use_preset(warp_actor, mesh_color_preset)
    ctrl.view_update()

@state.change("warp_color_preset")
def update_warp_color_preset(warp_color_preset, **kwargs):
    use_preset(warp_actor, warp_color_preset)
    ctrl.view_update()
...
@state.change("warp_opacity")
def update_mesh_opacity(warp_opacity, **kwargs):
    warp_actor.GetProperty().SetOpacity(warp_opacity)
    ctrl.view_update()

# Warp scale Callbacks
@state.change("warp_scale")
def update_warp_scale(warp_scale, **kwarges):
    ...
    # Warp: Color by default array
    warp_mapper.SelectColorArray(default_array.get("text"))
    warp_mapper.GetLookupTable().SetRange(default_min, default_max)
    if default_array.get("type") == vtkDataObject.FIELD_ASSOCIATION_POINTS:
        warp_mapper.SetScalarModeToUsePointFieldData()
    else:
        warp_mapper.SetScalarModeToUseCellFieldData()
    warp_mapper.SetScalarVisibility(True)
    warp_mapper.SetUseLookupTableScalarRange(True)
    ...
    trame.GitTree(
        sources=(
            ...
            [
                ...
                {"id": "2", "parent": "1", "visible": 1, "name": "Wrap"},
            ],
        ),
...
def wrap_card():
    with ui_card(title="Warp", ui_name="warp"):
        vuetify.VSlider(
            # Opacity
